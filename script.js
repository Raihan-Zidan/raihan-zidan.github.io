...
function instantanswer() {
  if (searchInput.value.toLowerCase().match(/jam|waktu|time|clock/) && searchInput.value.length < 15 && searchInput.value.split(" ").length - 4) {
    ...
    const countries={"am-ET":"Amharic","ar-SA":"Arabic","be-BY":"Bielarus","bn-IN":"Bengali","bs-BA":"Bosnian","ca-ES":"Catalan","cs-CZ":"Czech","cy-GB":"Welsh","da-DK":"Danish","de-DE":"German","dv-MV":"Maldivian","el-GR":"Greek","en-GB":"English","es-ES":"Spanish","et-EE":"Estonian","eu-ES":"Basque","fa-IR":"Persian","fi-FI":"Finnish","fn-FNG":"Fanagalo","fr-FR":"French","gl-ES":"Galician","gu-IN":"Gujarati","ha-NE":"Hausa","he-IL":"Hebrew","hi-IN":"Hindi","hr-HR":"Croatian","hu-HU":"Hungarian","id-ID":"Indonesian",su:"Sundanese",jv:"Javanese","is-IS":"Icelandic","it-IT":"Italian","ja-JP":"Japanese","kk-KZ":"Kazakh","km-KM":"Khmer","kn-IN":"Kannada","ko-KR":"Korean","ku-TR":"Kurdish","ky-KG":"Kyrgyz","la-VA":"Latin","lo-LA":"Lao","lv-LV":"Latvian","mg-MG":"Malagasy","mi-NZ":"Maori","ms-MY":"Malay","mt-MT":"Maltese","my-MM":"Burmese","ne-NP":"Nepali","nl-NL":"Dutch","no-NO":"Norwegian","ny-MW":"Nyanja","ur-PK":"Pakistani","pa-IN":"Panjabi","ps-PK":"Pashto","pl-PL":"Polish","pt-PT":"Portuguese","ro-RO":"Romanian","ru-RU":"Russian","si-LK":"Sinhala","sk-SK":"Slovak","sm-WS":"Samoan","sn-ZW":"Shona","so-SO":"Somali","sq-AL":"Albanian","sr-RS":"Serbian","sv-SE":"Swedish","sw-SZ":"Swahili","ta-LK":"Tamil","te-IN":"Telugu","tg-TJ":"Tajik","th-TH":"Thai","ti-TI":"Tigrinya","tk-TM":"Turkmen","tl-PH":"Tagalog","to-TO":"Tongan","tr-TR":"Turkish","uk-UA":"Ukrainian","uz-UZ":"Uzbek","vi-VN":"Vietnamese","xh-ZA":"Xhosa","yi-YD":"Yiddish","zu-ZA":"Zulu"};setTimeout(()=>{let e=document.querySelector(".trnsl"),a=e.querySelector(".from-text"),n=e.querySelector(".to-text"),i=e.querySelector(".exchange"),t=e.querySelectorAll("select"),l=e.querySelectorAll(".row i");function r(){let e=a.value.trim(),i=t[0].value,l=t[1].value;if(e){n.setAttribute("placeholder","Translating...");var r="https://translate.googleapis.com/translate_a/single?client=gtx&sl="+i+"&tl="+l+"&dt=t&q="+encodeURI(e),s="";s="",$.getJSON(r,function(e){for(var a=0;a<e[0].length;a++)s+=e[0][a][0],n.value=s})}}transButton=e.querySelector("button"),t.forEach((e,a)=>{for(let n in countries){let i=`<option ${0==a?"en-GB"==n?"selected":"":"id-ID"==n?"selected":""} value="${n}">${countries[n]}</option>`;e.insertAdjacentHTML("beforeend",i)}}),t.forEach(e=>{e.addEventListener("change",()=>{r()})}),i.addEventListener("click",()=>{let e=a.value,i=t[0].value;a.value=n.value,n.value=e,t[0].value=t[1].value,t[1].value=i,r()}),a.addEventListener("keyup",()=>{a.value||(n.value="")});let s,u=500;a.addEventListener("input",()=>{clearTimeout(s),a.value?s=setTimeout(r,u):a.value||n.setAttribute("placeholder","Translation")}),l.forEach(e=>{e.addEventListener("click",({target:e})=>{if(a.value&&n.value){if(e.classList.contains("fa-copy"))"from"==e.id?navigator.clipboard.writeText(a.value):navigator.clipboard.writeText(n.value);else{let i;"from"==e.id?(i=new SpeechSynthesisUtterance(a.value)).lang=t[0].value:(i=new SpeechSynthesisUtterance(n.value)).lang=t[1].value,speechSynthesis.speak(i)}}})})},500);
  }
}
...
function webresult(res) {
  try {
    ...
    for (var i = 0; i < res.items.length; i++) {
      ...
      document.querySelector(".main-result .result").innerHTML += `<div class="tab-result"><div class="tab-link"  data-number="${i}"><a href="${res.items[i].link}"><div class="top"><img src="https://t0.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=https://${originurl.hostname}&size=32" class="favicon"><div class="link">${displayUrl}</div></div><div class="title">${res.items[i].htmlTitle?.replace(/<b(?!\/b)>|<\/b>/g, "")}</div></a></div><div class="snippet">${res.items[i].htmlSnippet?.replace(/<b(?!\/b)>|<\/b>/g, "")}</div></div>`;
    }
