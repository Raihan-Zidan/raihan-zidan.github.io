...
<div class="search-item"><a href="/search?q=${encodeURIComponent(q).replace(/\%20/g,'+')}&tbm=nws${searchlang}" class="tab-wrapper" tab-id="news"><div class="label"><svg width="16" height="16" viewBox="0 0 22 22" fill="#6e7780"><path d="M12 11h6v2h-6v-2zm-6 6h12v-2H6v2zm0-4h4V7H6v6zm16-7.22v12.44c0 1.54-1.34 2.78-3 2.78H5c-1.64 0-3-1.25-3-2.78V5.78C2 4.26 3.36 3 5 3h14c1.64 0 3 1.25 3 2.78zM19.99 12V5.78c0-.42-.46-.78-1-.78H5c-.54 0-1 .36-1 .78v12.44c0 .42.46.78 1 .78h14c.54 0 1-.36 1-.78V12zM12 9h6V7h-6v2"></path></svg><span>News</span></div></a></div>
...
function nwsr(res) {
  try {
    for (var i = 0; i < res.items.length; i++) {
      ...
      publishtimes = (res.items[i].pagemap.metatags[0]['article:published_time'].slice(0, 4).match(d.getFullYear())) ? timeAgo(res.items[i].pagemap.metatags[0]['article:published_time']) : new Date(res.items[i].pagemap.metatags[0]['article:published_time'].slice(0, 10)).toLocaleString('en-US', {day: '2-digit', year: 'numeric', month: 'long'});
      publishtime = (res.items[i].pagemap.metatags[0]['article:published_time']) ? publishtimes : "Published";
    ...
    if (res.items.length < 1) document.querySelector(".main-result").innerHTML += `<div class="tab-result"><div class="title-black">${langtext("noresult")}</div><div class="suggestion">${langtext("suggtext")}</div><div>${langtext("noresultsug")}</div></div>`;
  }
}
