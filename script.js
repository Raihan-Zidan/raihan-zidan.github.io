  ...
  try {
    ...
    : '';
      ...
      var isThumb = (!bigThumb && res.items[i]?.snippet?.length > 150 && res.items[i]?.pagemap?.cse_thumbnail) ? `<div class="thumbnail"><img src="${res.items[i].pagemap.cse_thumbnail[0].src}"></div>` : "";
           document.querySelector(".main-result .result").insertAdjacentHTML('beforeend', `<div class="VtuHV Kj7VF tab-result eb8xCva" ${fdta}><div class="CeWka NbkAw"><div class="tab-link"  data-number="${i}"><a href="${res.items[i].link}"><div class="top"><div class="favicon"><img src="https://datasearch.searchdata.workers.dev/favicon?url=https://${originurl.hostname}"></div><div class="link-rw"><div class="link">${siteName}</div><div class="link k">https://${res.items[i].displayLink}</div></div></div><div class="title">${escapeHTML(res.items[i].title)}</div></a></div>${bigThumb}<div class="btm-snpt"><div class="snippet"><span>${escapeHTML(res.items[i].snippet) || langtext("noSiteInfo")}</span></div>${hasPri}${showLinks(res.items[i].link)}</div></div></div>`);
    }
