...
var searchString = "&svid=M6TaKOHpx71oPXzNrkZyQvetkoRpo5";
var rested = false;
...
<span>Maps</span></div></a></div></div></div><div class="result-wrapper"><div class="main-result"></div></div></div>`;
  document.querySelector(".search-input").setAttribute("value", q);
} else if (rested) {
...
searchToggle.addEventListener('click', ()=> {
  ...
  if (searchInput.value.trim()) {
    post_data(`/search?q=${encodeURIComponent(searchInput.value).replace(/\%20/g,'+')}${searchdata}${searchlang}${searchString}`);
  }
});
...
function nwsresult(res) {
  if (res.items.length > 5) {
    ...
    for (var i = 0; i < res.items.length; i++) {
      var thumbnailimg = (res.items[i].pagemap.cse_thumbnail) ? res.items[i].pagemap.cse_thumbnail[0].src : "error";
    ...
    document.querySelectorAll(".thumbnail[src='error']").forEach(elm => {
...
function webresult(res) {
  try {
    ...
    if (res.items && res.spelling && pageone) {
      document.querySelector(".main-result .result").innerHTML += `<div class="corrected-word tab-result"><div class="snippet">${langtext("correct")} <a class="spelling" href="/search?q=${encodeURIComponent(res.spelling.correctedQuery).replace(/\%20/g,'+')}${searchlang}${searchString}">${res.spelling.correctedQuery}</a></div></div>`;
    }
