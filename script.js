...
function instantanswer() {
  c = ["clock","jam"];
  if (searchInput.value.toLowerCase().match(c[0]) && searchInput.value.length < 15 && searchInput.value.split(" ").length - 4 || searchInput.value.toLowerCase().indexOf(c[1]) > -1 && searchInput.value.length < 15 && searchInput.value.split(" ").length - 4) {
