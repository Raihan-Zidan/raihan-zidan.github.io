...
function instant(e) {
  setTimeout(()=> {
    ...
    if (res.Abstract.length > 100) {
      ...
      document.querySelector(".instant-answer").innerHTML = `${thumbmg}<div class="title">${res.Heading}</div><div class="about"><span class="snippet">${res.Abstract.substring(res.Abstract.lastIndexOf("/>") + 1, res.Abstract.length).slice(0, 220)}... </span><a href="${res.AbstractURL}" class="wikipedia" title="Wikipedia">${res.AbstractSource}</a></div><div class="infobox"></div>`;
