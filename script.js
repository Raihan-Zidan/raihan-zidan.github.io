...
function nwsresult(res) {
  if (res.items.length > 3) {
    ...
    for (var i = 0; i < res.items.length; i++) {
      var gambarkan = (res.items[i].pagemap.cse_thumbnail) ? res.items[i].pagemap.cse_thumbnail[0].src : "/images/blank.png";
      var thumbnailimg = (res.items[i].pagemap.newsarticle[0].image) ? res.items[i].pagemap.newsarticle[0].image : gambarkan;
