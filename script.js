...
function nwsresult(res) {
  if (res.items.length > 3) {
    ...
    for (var i = 0; i < res.items.length; i++) {
      var errimg = (res.items[i].pagemap.newsarticle[0].image) ? res.items[i].pagemap.newsarticle[0].image : "https://play-lh.googleusercontent.com/CT1M2pKlUhGx4w5UHqarn6oSU_sa7L7XRW2-hQrfNi9oou6W81PbJnWi-9PbEfC_3g";
      var thumbnailimg = (res.items[i].pagemap.cse_thumbnail) ? res.items[i].pagemap.cse_thumbnail[0].src : errimg;
