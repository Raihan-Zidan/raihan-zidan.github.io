...
function instantanswer() {
  if (searchInput.value.toLowerCase().match(/jam|waktu|time|clock/) && searchInput.value.length < 15 && searchInput.value.split(" ").length - 4) {
    ...
    const countries={af:"Afrikaans",sq:"Albanian",am:"Amharic",ar:"Arabic",hy:"Armenian",as:"Assamese",ay:"Aymara",az:"Azerbaijani",bm:"Bambara",eu:"Basque",be:"Belarusian",bn:"Bengali",bho:"Bhojpuri",bs:"Bosnian",bg:"Bulgarian",ca:"Catalan",ceb:"Cebuano",co:"Corsican",hr:"Croatian",cs:"Czech",da:"Danish",nl:"Dutch",en:"English",eo:"Esperanto",et:"Estonian",ee:"Ewe",fil:"Filipino",fi:"Finnish",fr:"French",fy:"Frisian",ga:"Irish",gl:"Galician",de:"German",el:"Greek",gu:"Gujarati",ha:"Hausa",he:"Hebrew",hi:"Hindi",hu:"Hungarian",is:"Icelandic",ig:"Igbo",id:"Indonesian",it:"Italian",ja:"Japanese",jw:"Javanese",kn:"Kannada",kk:"Kazakh",km:"Khmer",ko:"Korean",ky:"Kyrgyz",lo:"Lao",lv:"Latvian",lt:"Lithuanian",lu:"Lushootseed",mk:"Macedonian",mg:"Malagasy",ms:"Malay",ml:"Malayalam",mt:"Maltese",mr:"Marathi",mn:"Mongolian",my:"Burmese",ne:"Nepali",no:"Norwegian",pa:"Punjabi",pl:"Polish",pt:"Portuguese",ro:"Romanian",ru:"Russian",sa:"Sanskrit",si:"Sinhala",sk:"Slovak",sl:"Slovenian",so:"Somali",es:"Spanish",su:"Sundanese",sw:"Swahili",sv:"Swedish",ta:"Tamil",te:"Telugu",th:"Thai",tr:"Turkish",uk:"Ukrainian",ur:"Urdu",uz:"Uzbek",vi:"Vietnamese",xh:"Xhosa",yo:"Yoruba",zu:"Zulu"},trnsbox=document.querySelector(".trnsl"),fromText=trnsbox.querySelector(".from-text"),toText=trnsbox.querySelector(".to-text"),swapButton=trnsbox.querySelector(".exchange"),selectTag=trnsbox.querySelectorAll("select"),icons=trnsbox.querySelectorAll(".row i");function translateText(){let e=fromText.value.trim(),a=selectTag[0].value,t=selectTag[1].value;if(e){toText.setAttribute("placeholder","Translating..."),toText.value&&(toText.value=`${toText.value} ...`);var n="",l="https://translate.googleapis.com/translate_a/single?client=gtx&sl="+a+"&tl="+t+"&dt=t&q="+encodeURI(e);$.getJSON(l,function(e){for(var a=0;a<e[0].length;a++)n+=e[0][a][0],toText.value=n;toText.value&&swapButton.classList.remove("off")})}}transButton=trnsbox.querySelector("button"),selectTag.forEach((e,a)=>{for(let t in countries){let n=`<option ${0==a?"en"==t?"selected":"":"id"==t?"selected":""} value="${t}">${countries[t]}</option>`;e.insertAdjacentHTML("beforeend",n)}}),selectTag.forEach(e=>{e.addEventListener("change",()=>{translateText()})}),swapButton.addEventListener("click",()=>{fromText.value;let e=selectTag[0].value;fromText.value=toText.value,selectTag[0].value=selectTag[1].value,selectTag[1].value=e,translateText()}),fromText.addEventListener("keyup",()=>{fromText.value||(toText.value="")});let typingTimer,doneTypingInterval=500;fromText.addEventListener("input",()=>{clearTimeout(typingTimer),fromText.value?typingTimer=setTimeout(translateText,doneTypingInterval):fromText.value||toText.setAttribute("placeholder","Translation"),fromText.value&&!toText.value?swapButton.classList.add("off"):swapButton.classList.remove("off")}),icons.forEach(e=>{e.addEventListener("click",({target:e})=>{if(fromText.value&&toText.value){if(e.classList.contains("fa-copy"))"from"==e.id?navigator.clipboard.writeText(fromText.value):navigator.clipboard.writeText(toText.value);else{let a;"from"==e.id?(a=new SpeechSynthesisUtterance(fromText.value)).lang=selectTag[0].value:(a=new SpeechSynthesisUtterance(toText.value)).lang=selectTag[1].value,speechSynthesis.speak(a)}}})});
  }
}
