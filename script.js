...
function hndlr(res) {
  try {
    ...
    for (var i = 0; i < res.items.length; i++) {
      allDetails = res.items[i];
      let {title, snippet, displayLink} = allDetails;
      document.getElementById("hasil").innerHTML += `<div class="tab-result"><div class="tab-link"><a href="${res.items[i].link}"><div class="top"><img src="https://t0.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=${res.items[i].link}&size=64" class="favicon"><div class="link">${displayLink}</div></div><div class="title">${title}</div></a></div><div class="snippet">${res.items[i].htmlSnippet}</div></div>`;
    }
